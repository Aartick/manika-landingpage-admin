...
export default function IncludedSectionEdit({
  includedSection,
  setIncludedSection,
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {includedSection.items.map((item, idx) => (
            <div key={idx} className="card p-6 border border-[#C2A570]/30 rounded-2xl">

              {/* Icon is fixed (string), shown for info */}
              <p className="text-dark-brown font-semibold mb-2">{item.icon}</p>

              <input
                value={item.title}
                onChange={(e) => updateItem(idx, "title", e.target.value)}
                className="text-xl font-semibold mb-3 bg-transparent border-b border-[#C2A570] w-full"
              />
        ...
        <div className="mt-12 card bg-amber-50 p-6 rounded-2xl border border-[#C2A570]/30">
          <input
            ...
            className="text-2xl font-bold text-dark-brown mb-2 bg-transparent border-b border-[#C2A570] w-full"
          />
